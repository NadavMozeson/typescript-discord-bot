...
export const handleTeamSuggest = withErrorHandling(
...
const sendStartMessage = withErrorHandling(async () => {
  const channel = await client.channels.fetch(
    config.SERVER.CHANNELS.TeamRating
  );
  if (channel instanceof TextChannel) {
    const messageString =
      "# ⚒⚒️ מדריך לדירוג קבוצות ⚒️\n" +
    ...
    const attachment = new AttachmentBuilder(
      "./src/images/team-rating-banner.jpg"
    );
...
const deleteAllMessages = withErrorHandling(async () => {
  ...
  if (channel instanceof TextChannel) {
    ...
    const oldFetchedMessages = fetchedMessages.filter(
      (msg) => msg.createdTimestamp <= fourteenDaysAgo
    );
...
const preformRaffle = withErrorHandling(
  async (amount: number, interaction: CommandInteraction) => {
    const channel = await client.channels.fetch(
      config.SERVER.CHANNELS.TeamRating
    );
    const vipServer = await client.guilds.fetch(
      config.VIP_SERVER.INFO.ServerId
    );
    if (channel instanceof TextChannel) {
      ...
      for (const msg of messages) {
        ...
        const vipUser = await vipServer.members.fetch(msg[1].author.id);
        if (user instanceof GuildMember) {
          if (!user.permissions.has(PermissionsBitField.Flags.Administrator)) {
            if (
              vipUser &&
              (vipUser.roles.cache.has(config.VIP_SERVER.ROLES.VIP) ||
                vipUser.roles.cache.has(config.VIP_SERVER.ROLES.VIP2))
            ) {
...
const sendEndMessage = withErrorHandling(async () => {
  ...
  if (channel instanceof TextChannel) {
    const messageString =
      "# ⛔ נסגר מחזור שדרוג הקבוצות הנוכחי ⛔\n" +

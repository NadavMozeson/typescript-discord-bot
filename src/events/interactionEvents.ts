...
import {
  ...
  createNewInvestment,
  deleteInvestment,
  postEarlyExitMessage,
  postFirstExitMessage,
  postNewFoderInvestment,
  postNewInvestment,
  postNewTOTWInvestment,
  postProfitMessage,
  sendInvestmentListPicker,
} from "../assets/newInvestments.js";
import {
  generateTrackerListMessage,
  handelTrackerButtonClick,
} from "../assets/investmentTracker.js";
...
export async function setupInteractionEvents() {
  client.on(
    "interactionCreate",
    withErrorHandling(async (interaction: Interaction) => {
...
const handleButtons = withErrorHandling(
  async (interaction: ButtonInteraction) => {
    if (
      interaction.customId.toString().includes("ticket") &&
      isPrimaryServer(interaction)
    ) {
      await handleTicketButtons(interaction);
    } else if (interaction.customId.toString().includes("tracker_button_")) {
      await handelTrackerButtonClick(interaction);
    } else if (
      interaction.customId.toString().includes("faq-click-") &&
      isPrimaryServer(interaction)
    ) {
      await handleNewFAQClick(interaction);
    } else if (
      interaction.customId.toString().includes("confirm_delete_inv_")
    ) {
...
const handleSlashCommands = withErrorHandling(
  async (interaction: CommandInteraction) => {
    if (interaction.commandName === "open-dm" && isPrimaryServer(interaction)) {
      ...
      interaction.commandName === "profit" ||
      interaction.commandName === "exit" ||
      interaction.commandName === "first-exit" ||
      interaction.commandName === "delete_investment"
    ) {
      ...
      interaction.commandName === "team-suggest" &&
      isPrimaryServer(interaction)
    ) {
      await handleTeamSuggest(interaction);
    } else if (
      interaction.commandName === "faq-new" &&
      isPrimaryServer(interaction)
    ) {
...
const handleSelectMenuInteraction = withErrorHandling(

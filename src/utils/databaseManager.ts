...
const bulkDeleteDataFromDatabase = async (
...
const getAllDataWithSearchInDatabase = async (
  collectionName: string,
  data: any
) => {
  const collection = collections[collectionName];
...
class PrivateChatManager {
  ...
  async createNewChat(
    ...
    await this.privateDMHandler.addData({
      user: userID,
      channel: channelID,
      VIP: isVIP,
    });
...
class InvestmentsManager {
  ...
  async deleteInvestmentByID(id: string) {
    ...
    const deletedData = await this.investmentsHandler.deleteData({
      _id: new ObjectId(id),
    });
    return deletedData;
  }
...
class InvestmentsTrackerManager {
  ...
  async createNewTracker(userID: string, investmentID: string) {
    return await this.InvestmentsTrackerHandler.addData({
      user: userID,
      investment: investmentID,
    });
  }
  async deleteTracker(userID: string, investmentID: string) {
    return await this.InvestmentsTrackerHandler.deleteData({
      user: userID,
      investment: investmentID,
    });
  }
  async checkIfExists(userID: string, investmentID: string) {
    return await this.InvestmentsTrackerHandler.checkIfExists({
      user: userID,
      investment: investmentID,
    });
  }
  async getAllTrackerOfInvestment(investmentID: string) {
    return await this.InvestmentsTrackerHandler.getAllDataWithSearch({
      investment: investmentID,
    });
  }
  async deleteAllTrackerOfInvestment(investmentID: string) {
    await disableInvestmentButtons(investmentID);
    return await this.InvestmentsTrackerHandler.bulkDeleteData({
      investment: investmentID,
    });
  }
  async getUserTrackerInvestments(userID: string) {
    const investments =
      await this.InvestmentsTrackerHandler.getAllDataWithSearch({
        user: userID,
      });
    let result = [];
    for (const investment_id of investments) {
      result.push(
        await dbManager.Investments.getInvestmentByID(investment_id.investment)
      );
    }
    return result;
  }
...
class FAQManager {
  ...
  async createNewQuestion(question: string, answer: string) {
    return await this.FAQHandler.addData({
      question: question,
      answer: answer,
    });
  }
